...
public class PaymentDoRequestTest {
    ...
    void 결제금액과_결제수단은_필수값이며_없으면_실패한다() {
        InvalidPaymentDoParameterException error1 = assertThrows(
                InvalidPaymentDoParameterException.class,
                () -> new PaymentDoRequest.Item(PaymentMethod.CARD, null));
        assertEquals(
                PaymentErrors.INVALID_PAYMENT_PARAMETERS.getCode(),
                error1.getErrorCode());
        assertTrue(error1.getMessage().contains("결제금액이 비어있습니다."));

        InvalidPaymentDoParameterException error2 = assertThrows(
                InvalidPaymentDoParameterException.class,
    ...
    void 결제금액은_0을_포함한_양수여야한다() {
        InvalidPaymentDoParameterException error = assertThrows(
                InvalidPaymentDoParameterException.class,
                () ->  new PaymentDoRequest.Item(PaymentMethod.CARD, -1_000));
        assertEquals(PaymentErrors.INVALID_PAYMENT_PARAMETERS.getCode(), error.getErrorCode());
    ...
    void 주문ID가_존재해야_한다() {
        ...
        InvalidPaymentDoParameterException error3 = assertThrows(
                InvalidPaymentDoParameterException.class,
